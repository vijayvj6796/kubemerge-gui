...
export default function App() {
  ...
  useEffect(() => {
    ...
    // ── KEY CHANGE: resize window when switching modes ─────────────────
    const unlistenSet = EventsOn("set-floating-mode", async (enabled: boolean) => {
      setFloatingModeEnabled(enabled);
      setShowFloatingWidget(enabled);
      if (!enabled) {
        // Switching TO full GUI — restore window to full size
        try { await WidgetFullGUI(); } catch {}
      }
    });
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape") {
        if (dropdownOpen) { setDropdownOpen(false); setSearchTerm(""); }
        if (namespaceDropdownOpen) { setNamespaceDropdownOpen(false); setNamespaceSearchTerm(""); }
      }
    };
  ...
  async function loadFromTarget(kind: TargetKind, distro: string, user: string) {
    try {
      ...
      if (kind === "wsl") {
        if (!distro.trim()) { setStatus("Error: WSL distro is required"); return; }
        if (!user.trim())   { setStatus("Error: Linux username is required"); return; }
      }
      const path = await ResolveTargetKubeconfig({
        kind, distro: distro.trim(), linuxUser: user.trim(),
      } as any);
      ...
      if (!ok) { setStatus(`Target load failed: kubeconfig not found at ${path}`); return; }
  ...
  async function doSwitch() {
    ...
    try {
      if (!targetPath) { setStatus("Target not loaded. Click Load Target."); return; }
  ...
  async function testClusterConnection() {
    if (!targetPath || !currentContext) { setStatus("No context selected. Load target first."); return; }
    ...
    try {
      ...
      setClusterInfo({
        reachable: false, version: "", authenticated: false,
        clusterName: currentContext, serverUrl: "",
  ...
  return (
    ...
    >
      ...
      {floatingModeEnabled ? (
        ...
        /* ── FULL GUI MODE ───────────────────────────────────────────── */
        <>
          {/* Custom title bar — needed because window is Frameless: true */}
          <TitleBar />
          ...
          <div className="grid">
              ...
              <div className="row" style={{ marginTop: "14px" }}>
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (path) { setFilePath(path); setStatus(`Selected: ${path}`); }
                  }}
                >
                  📁 Select kubeconfig
                </button>
              ...
              {result && (
                <>
                  ...
                  <div className="infoBox infoBox-success" style={{ marginBottom: "12px" }}>
                    ...
                    <div style={{ fontSize: "12px", opacity: 0.9 }}>{result.message}</div>
                  </div>
                  <div className="label">Target</div>
                  <div className="pathbox">{result.targetConfigPath}</div>
                  <div className="label" style={{ marginTop: "12px" }}>Backup</div>
            ...
            <div className="card">
                ...
                {os === "windows" ? (
                  <>
                    <label style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                      <input type="radio" checked={targetKind === "windows"} onChange={() => setTargetKind("windows")} />
                      Windows
                    </label>
                    <label style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                      <input type="radio" checked={targetKind === "wsl"} onChange={() => setTargetKind("wsl")} />
                      WSL
                    </label>
                  </>
                ) : (
                  <label style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                    <input type="radio" checked={targetKind === "linux"} onChange={() => setTargetKind("linux")} />
                    Linux (~/.kube/config)
                  </label>
                )}
              </div>
                  ...
                  <div className="row" style={{ marginBottom: "10px" }}>
                    <input
                      style={{ border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.06)",
                        color: "rgba(255,255,255,0.92)", borderRadius: "12px", padding: "10px 12px" }}
                      value={linuxUser} onChange={(e) => setLinuxUser(e.target.value)}
                      placeholder="e.g. vj"
                    />
                  </div>
                  {wslDistro && linuxUser && (
                    <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.6)", marginBottom: "10px", fontFamily: "monospace" }}>
              ...
              <div className="row" style={{ marginBottom: "10px" }}>
                <button className="btnPrimary" onClick={async () => await loadFromTarget(targetKind, wslDistro, linuxUser)}>
                  🔄 Load Target
                </button>
              </div>
              ...
              <div className="row">
                <div className="dropdown">
                  <div className="dropdownSelected" onClick={() => setDropdownOpen(!dropdownOpen)}>
                  ...
                  {dropdownOpen && (
                    <div className="dropdownMenu">
                      <div className="dropdownSearch">
                        <input ref={searchInputRef} type="text" placeholder="🔍 Search contexts..."
                          value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} autoFocus />
                      </div>
                      {filteredContexts.length === 0 && (
                        <div className="dropdownItem" style={{ opacity: 0.6, cursor: "default" }}>No contexts found</div>
                      )}
                      {filteredContexts.map((c) => (
                        <div key={c} className={`dropdownItem ${c === currentContext ? "active" : ""}`}
                          onClick={() => { setSelectedContext(c); setDropdownOpen(false); setSearchTerm(""); }}>
                ...
                <button className="btnSuccess" disabled={!selectedContext || selectedContext === currentContext} onClick={doSwitch}>
                  ✓ Switch
                </button>
              </div>
              ...
              <div className="row">
                <div className="dropdown">
                  <div className="dropdownSelected"
                    onClick={() => { if (allNamespaces.length === 0) loadNamespaces(); setNamespaceDropdownOpen(!namespaceDropdownOpen); }}>
                  ...
                  {namespaceDropdownOpen && (
                    <div className="dropdownMenu">
                      <div className="dropdownSearch">
                        <input ref={namespaceSearchInputRef} type="text" placeholder="🔍 Search namespaces..."
                          value={namespaceSearchTerm} onChange={(e) => setNamespaceSearchTerm(e.target.value)} autoFocus />
                      </div>
                      {loadingNamespaces && (
                      ...
                      {!loadingNamespaces && filteredNamespaces.length === 0 && (
                        <div className="dropdownItem" style={{ opacity: 0.6, cursor: "default" }}>No namespaces found</div>
                      )}
                      {!loadingNamespaces && filteredNamespaces.map((ns) => (
                        <div key={ns} className={`dropdownItem ${ns === currentNamespace ? "active" : ""}`}
                          onClick={() => { setSelectedNamespace(ns); setNamespaceDropdownOpen(false); setNamespaceSearchTerm(""); }}>
              ...
              <button className="btnGhost" disabled={!currentContext || loadingDetails}
                onClick={loadContextDetails} style={{ width: "100%", marginBottom: "10px" }}>
                {loadingDetails ? (
              ...
              {contextDetails && (
                <div className="infoBox infoBox-neutral" style={{ marginBottom: "10px" }}>
                  ...
                  <div style={{ marginBottom: "8px" }}><strong>Cluster:</strong> <code>{contextDetails.clusterName}</code></div>
                  ...
                  <div style={{ marginBottom: "8px" }}><strong>User:</strong> <code>{contextDetails.userName}</code></div>
                  <div style={{ marginBottom: "8px" }}><strong>Namespace:</strong> <code>{contextDetails.namespace}</code></div>
                  {contextDetails.hasCertExpiration && (
                    <>
                      ...
                      {contextDetails.certExpirationWarning && (
                        <div className={`infoBox ${contextDetails.certExpiresInDays < 0 ? "infoBox-error" : "infoBox-warning"}`}
                          style={{ marginTop: "10px", padding: "10px 12px", fontSize: "12px", fontWeight: 600 }}>
                          {contextDetails.certExpirationWarning}
                        </div>
                      )}
                    </>
              ...
              <ul className="list" style={{ maxHeight: "300px", overflowY: "auto", padding: "8px 0 8px 8px", margin: 0, listStyle: "none" }}>
                {allContexts.slice(0, 15).map((c) => (
                  <li key={c} style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1 }}>
                      ...
                      <code style={{ fontSize: "12px", fontWeight: c === currentContext ? 600 : 400,
                        color: c === currentContext ? "var(--text)" : "var(--muted)" }}>{c}</code>
                    </div>
                    <button className="btnDanger" style={{ fontSize: 11, padding: "4px 10px", minWidth: "auto" }}
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (currentContext === c) { setCurrentContext(""); setSelectedContext(""); }
                        } catch (e: any) {
                ...
                {allContexts.length > 15 && (
                  <li style={{ padding: "8px 10px", textAlign: "center", opacity: 0.6, fontSize: "12px" }}>
                    …and {allContexts.length - 15} more contexts
                  </li>
                )}
                {allContexts.length === 0 && (
                  <li style={{ padding: "20px", textAlign: "center", opacity: 0.5, fontSize: "13px" }}>
              ...
              <button className="btnGhost" disabled={!currentContext || testingCluster}
                onClick={testClusterConnection} style={{ width: "100%", marginBottom: "10px" }}>
                {testingCluster ? (
                  <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              ...
              {clusterInfo && (
                <div className={`infoBox ${clusterInfo.reachable ? "infoBox-success" : "infoBox-error"}`}>
                  ...
                  {clusterInfo.reachable ? (
                    <>
                      <div style={{ marginBottom: "8px" }}><strong>Version:</strong> <code>{clusterInfo.version}</code></div>
                      <div style={{ marginBottom: "8px" }}><strong>Auth:</strong> <span style={{ color: "#4ade80" }}>✓ Authenticated</span></div>
                    ...
                    <div style={{ fontSize: "12px", lineHeight: 1.6 }}>{clusterInfo.errorMessage}</div>
                  )}
                </div>
              )}
              ...
              <div className="infoBox infoBox-info" style={{ fontSize: "12px" }}>
                {[["Ctrl+K","Quick context switcher"],["Ctrl+N","Quick namespace switcher"],
                  ["Ctrl+M","Quick merge"],["Ctrl+F","Search contexts/namespaces"],["Esc","Close dropdown"]
                ].map(([key, label]) => (
                  <div key={key} style={{ marginBottom: "6px" }}>
                    <kbd style={{ background: "rgba(0,0,0,0.3)", padding: "2px 6px", borderRadius: "4px", fontFamily: "monospace" }}>{key}</kbd>
                    {" "}{label}
                  </div>
                ))}
              </div>
      ...
      {/* Floating widget overlay (when in full GUI mode) */}
